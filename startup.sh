...
grep -q 'try_files \$uri /index.php?\$query_string;' /etc/nginx/sites-enabled/default || \
  sed -i '/location \/ {/a\        try_files $uri /index.php?$query_string;' /etc/nginx/sites-enabled/default
exit 0
